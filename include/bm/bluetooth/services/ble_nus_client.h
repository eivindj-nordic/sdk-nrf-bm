...
uint32_t ble_nus_client_init(struct ble_nus_client *nus_client,
			     struct ble_nus_client_config *nus_client_config);
 ...
 * @retval NRF_ERROR_INVALID_STATE If NUS Client connection handle or nus_tx_cccd_handle is invalid.
 * @return In addition, this function may return any error returned by the following functions:
 ...
 *        - @ref ble_gq_item_add()
 */
uint32_t ble_nus_client_string_send(struct ble_nus_client *nus_client, uint8_t *string,
				    uint16_t length);
