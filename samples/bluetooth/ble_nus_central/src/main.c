...
static void on_ble_evt(ble_evt_t const *ble_evt, void *context)
{
	...
	case BLE_GAP_EVT_CONNECTED:
		nrf_err = ble_nus_client_handles_assign(&nus_c, ble_evt->evt.gap_evt.conn_handle,
							 NULL);
		...
		nrf_err = ble_db_discovery_start(&db_disc, ble_evt->evt.gap_evt.conn_handle);
		if (nrf_err) {
...
static void on_db_disc_evt(struct ble_db_discovery *db_discovery, struct ble_db_discovery_evt *evt)
{
	ble_nus_client_on_db_disc_evt(&nus_c, evt);
}

static void on_scan_evt(struct ble_scan_evt const *scan_evt)
{
	uint32_t nrf_err;
	...
	case BLE_SCAN_EVT_CONNECTED:
		ble_gap_evt_connected_t const *connected = scan_evt->connected.connected;
...
static void on_nus_client_evt(struct ble_nus_client *ble_nus_c,
			      struct ble_nus_client_evt const *ble_nus_evt)
{
	...
	case BLE_NUS_CLIENT_EVT_TX_DATA:
		LOG_INF("BLE_NUS_CLIENT_EVT_TX_DATA event, len: %d",
...
static uint32_t nus_client_init(void)
{
	...
	struct ble_nus_client_config init = {
		.evt_handler = on_nus_client_evt,
		.gatt_queue = &gatt_queue,
		.db_discovery = &db_disc,
	};

	nrf_err = ble_nus_client_init(&nus_c, &init);
	if (nrf_err) {
...
static uint32_t scan_init(void)
{
	...
	struct ble_scan_config init_scan = {
		...
		.evt_handler = on_scan_evt,
	};

	nrf_err = ble_scan_init(&scan, &init_scan);
	if (nrf_err) {
	...
	nrf_err = ble_scan_filter_add(&scan, BLE_SCAN_UUID_FILTER, &uuid_filter);
	if (nrf_err) {
	...
	nrf_err = ble_scan_filter_add(&scan, BLE_SCAN_NAME_FILTER, &name_filter);
	if (nrf_err) {
		LOG_ERR("Failed to set filter, nrf_error %#x", nrf_err);
		return nrf_err;
	}
#endif
	nrf_err = ble_scan_filters_enable(&scan, BLE_SCAN_NAME_FILTER | BLE_SCAN_UUID_FILTER,
					  false);
	if (nrf_err) {
...
static uint32_t db_discovery_init(void)
{
	...
	nrf_err = ble_db_discovery_init(&db_disc, &db_cfg);
	if (nrf_err) {
...
int main(void)
{
	...
	if (err) {
		LOG_ERR("sdh_enable_request failed, err %d", err);
		goto idle;
	}
	...
	if (err) {
		LOG_ERR("sdh_enable_enable failed, err %d", err);
		goto idle;
	}
